...
%SubdividePath Takes a path: a vector consisting of m concatenated vectors in
%R^2n and returns a path of length 2m by insering the average of every two
%points between them
%   Parameters:
%   tx - x is the vector of derivatives, and tx is x transposed. We need x
%   to be transposed in order to use a reshaping trick that makes the
%   subdivision of the path easier.
x = tx';
